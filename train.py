...
from models import GCNNet, SAGE, RGCNNet, SEALNet, RGCNNetMultiTask
...
if __name__ == '__main__':
    ...
    print(args)
    # graph, edge_attr = AmazonFineFoodsReviews(database_path=args.input).build_graph(
    #     text_feature=False,
    #     language_model_name=args.language_model_shortcut,
    #     max_length=args.max_length)

    # with open("data/graph.pkl", "wb") as stream:
    #     pickle.dump(graph, stream)
    #
    # with open("data/edge_attr.pkl", "wb") as stream:
    #     pickle.dump(edge_attr, stream)
    ...
    # normalization
    graph.y = graph.y - 1
    graph.y = graph.y / (torch.max(graph.y) - torch.min(graph.y))
    ...
    if args.model in ['gcn', 'rgcn', 'sage']:
        ...
        for epoch in range(args.epoch):
            ...
            for cluster in cluster_data:
                ...
                train_loss = net.learn(data=cluster, optimizer=optim, criterion=criterion, edge_map=edge_map,
                                       device=args.device)
                val_loss, test_loss = net.evaluate(data=cluster, criterion=criterion, edge_map=edge_map,
                                                   device=args.device)
                ...
                # val_perf, temp_test_perf = net.evaluate(data=cluster, edge_map=edge_map, device=args.device)
            ...
            avg_test_loss = total_test_loss / cnt
            ...
            print(
                f"Epoch {epoch + 1:04d}/{args.epoch:04d}: train: {avg_train_loss:.5f} - val: {avg_val_loss:.5f} - test: {avg_test_loss:.5f}")
